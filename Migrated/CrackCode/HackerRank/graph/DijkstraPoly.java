...
public class DijkstraPoly {
	...
	public static int[][] floyd(final Adj[] g) {
		int N = g.length, OO = 1000000000;
		int[][] dist = new int[N][N];
		for (int src = 0; src < N; src++) {
			Arrays.fill(dist[src], OO);
			dist[src][src] = 0;
			for (E edge : g[src])
				dist[src][edge.dst] = edge.wgt;
		}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < N; i++) {
			g[i] = new Adj();
			for (int j = 0; j < N; j++) {
				if (adj[i][j] < 1000000000) g[i].add(new E(i, j, adj[i][j]));
			}
		}
		Q = I(in.nextLine());
