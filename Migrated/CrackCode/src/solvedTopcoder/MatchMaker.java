...
public class MatchMaker {
	public static String[] getBestMatches(String[] members, String currentUser,
			int sf) {
		Member allMember[] = new Member[members.length];
		int index = -1;
		for (int i = 0; i < allMember.length; i++) {
			Scanner in = new Scanner(members[i]);
			allMember[i] = new Member(i, in.next(), in.next(), in.next(),
					in.nextLine());
		...
		for (int i = 0; i < allMember.length; i++) {
			if (i != index
					&& allMember[i].gender
							.compareTo(allMember[index].requestedGender) == 0) {
				int count = 0;
				for (int j = 1; j < allMember[i].answer.length(); j += 2) {
					if (allMember[i].answer.charAt(j) == allMember[index].answer
							.charAt(j)) {
						count++;
					}
				}
	...
	static class Member implements Comparable<Member> {
		...
		public Member(int theIndex, String theName, String theGender,
				String theRequestedGender, String theAnswer) {
			this.index = theIndex;
			this.name = theName;
