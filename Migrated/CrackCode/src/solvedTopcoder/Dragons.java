...
public class Dragons {
	public String snaug(int[] initialFood, int rounds) {
		...
		for (int i = 0; i < rounds; i++) {
			...
			for (int j = 0; j < 6; j++) {
				...
				for (int k = 0; k < 4; k++) {
					nextFood[j] = Rational.add(nextFood[j],
							Rational.divideBy4(food[neighbors[j][k]]));
				}
			}
			food = nextFood;
		}
	...
	static class Rational {
		...
		static Rational divideBy4(Rational a) {
			return new Rational(a.numerator,
					a.denominator.multiply(new BigInteger(4 + "")));
		}
		...
		static Rational add(Rational a, Rational b) {
			...
			return new Rational(temp1.add(temp2),
